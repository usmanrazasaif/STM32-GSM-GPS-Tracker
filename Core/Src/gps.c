...
uint8_t rx_buffer_gps[GPSBUFSIZE];
...
void GPS_Init()
{
	HAL_UART_Receive_IT(GPS_USART, &rx_data_gps, 1);
}


void GPS_UART_CallBack(){
	if (rx_data_gps != '\n' && rx_index_gps < sizeof(rx_buffer_gps)) {
		rx_buffer_gps[rx_index_gps++] = rx_data_gps;
	} else {

		#if (GPS_DEBUG == 1)
		GPS_print((char*)rx_buffer_gps);
		#endif

		if(GPS_validate((char*) rx_buffer_gps))
			GPS_parse((char*) rx_buffer_gps);
		rx_index_gps = 0;
